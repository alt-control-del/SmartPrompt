...
# Streamlit app title
st.title("Effective Prompt Engineering with Google Gemini")
...
def call_gemini_model(api_key, user_prompt):
    try:
...
if st.button("Generate Refined Prompt"):
    if not user_api_key:
        ...
        with st.spinner("Refining your prompt..."):
            refined_prompt = call_gemini_model(user_api_key, user_prompt)
...
st.sidebar.markdown(
    """
    **Instructions**:
    1. Get your Google API key from the [Google AI Studios](https://ai.google.dev/aistudio).
    2. Ensure that your key has access to the Gemini model.
